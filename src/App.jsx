...
function App() {
  ...
  useEffect(() => {
    const updateTimes = () => {
      ...
      for (const key in timezones) {
        const localTime = new Date(now.toLocaleString("en-US", { timeZone: timezones[key] }));
        times[key] = format(localTime, "yyyy-MM-dd HH:mm:ss");
      }
  ...
  for (const key in timezones) {
    ...
    futureTimes[key] = format(new Date(localTime.getTime() + diff), "yyyy-MM-dd HH:mm");
  }
